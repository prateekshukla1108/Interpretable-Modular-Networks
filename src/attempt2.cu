...
public:
    ...
    Tensor(int r, int c, bool half) : rows(r), cols(c), size(r * c), is_half(half) {
        if (is_half) {
            if (size > 0) checkCudaErrors(cudaMalloc(&data, size * sizeof(__half)));
            else data = nullptr;
        } else {
            if (size > 0) checkCudaErrors(cudaMalloc(&data, size * sizeof(float)));
            else data = nullptr;
        }
        if (data) checkCudaErrors(cudaMemset(data, 0, size * (is_half ? sizeof(__half) : sizeof(float))));
    }
    ...
    void to_device(const float* host_data, cudaStream_t stream, bool normalize = false) {
        ...
        if (is_half) {
            ...
            if (size > 0) checkCudaErrors(cudaMemcpyAsync(data, h_data.data(), size * sizeof(__half), cudaMemcpyHostToDevice, stream));
        } else {
            if (size > 0) checkCudaErrors(cudaMemcpyAsync(data, temp.data(), size * sizeof(float), cudaMemcpyHostToDevice, stream));
        }
    }
    void to_host(float* host_data, cudaStream_t stream) const {
        if (is_half) {
            std::vector<__half> h_data(size);
            checkCudaErrors(cudaMemcpyAsync(h_data.data(), data, size * sizeof(__half), cudaMemcpyDeviceToHost, stream));
            checkCudaErrors(cudaStreamSynchronize(stream));
            for (int i = 0; i < size; ++i) {
                host_data[i] = __half2float(h_data[i]);
            }
        } else {
            checkCudaErrors(cudaMemcpyAsync(host_data, data, size * sizeof(float), cudaMemcpyDeviceToHost, stream));
            checkCudaErrors(cudaStreamSynchronize(stream));
        }
    }
};
...
public:
    ...
    int get_cols() const { return data.cols; }
};

void update_parameter(Parameter& param, float lr, float beta1, float beta2,
                     float epsilon, int step, cudaStream_t stream);
...
__global__ void adam_update_kernel_half(
    __half* param, const __half* grad, __half* m, __half* v, int size, int step,
    ...
    if (idx < size) {
        ...
        float m_hat = m_val / (1 - powf(beta1, step));
        float v_hat = v_val / (1 - powf(beta2, step));
...
__global__ void cross_entropy_loss_forward_kernel_half(const __half* predictions, const int* labels, float* loss, int batch_size, int num_classes) {
    if (blockIdx.x == 0 && threadIdx.x == 0) {
        ...
        float max_val_batch = -1e38f;
        for (int i = 0; i < batch_size; ++i) {
            int label = labels[i];
            float log_prob = -1e38f;
            float max_val = -1e38f;
            for (int j = 0; j < num_classes; ++j) {
                max_val = fmaxf(max_val, __half2float(predictions[i * num_classes + j]));
            }
             max_val_batch = fmaxf(max_val_batch, max_val);
            float sum_exp = 0.0f;
            for (int j = 0; j < num_classes; ++j) {
         ...
         if (isnan(max_val_batch) || isinf(max_val_batch)) {
            printf("Warning: NaN or Inf detected in predictions in cross_entropy_loss_forward_kernel_half!\n");
        }
...
void update_parameter(Parameter& param, float lr, float beta1, float beta2,
    ...
    adam_update_kernel_half<<<blocks, threads, 0, stream>>>(
        ...
        param.data.size, step, lr, beta1, beta2, epsilon
    );
}
...
public:
    ...
    void backward(const Tensor& input, const Tensor& grad_output,
        ...
        checkCudaErrors(cudaGetLastError());
        int threads = 256;
        int blocks = (biases.get_cols() + threads - 1) / threads;
...
public:
    ...
    void backward(const Tensor& input, const Tensor& grad_output,
                    Tensor& grad_input, cudaStream_t stream) {
        Tensor linear_output(input.rows, linear.output_dim, true);
        linear.forward(input, linear_output, stream);
...
public:
    ...
    AttentionLayer(const AttentionLayer&) = default;
    AttentionLayer& operator=(const AttentionLayer&) = default;
    ...
    void backward(const Tensor& grad_attention_weights, cudaStream_t stream) {
        softmax_backward_kernel_half<<<1, 1, 0, stream>>>(
...
class AlignmentLoss {
public:
    AlignmentLoss() = default;
    AlignmentLoss(const AlignmentLoss&) = default;
    AlignmentLoss& operator=(const AlignmentLoss&) = default;
    float compute(const Tensor& attention_weights, cudaStream_t stream, const Hyperparameters& hp) {
        ...
        return hp.lambda1 * (sum_sq_diff / attention_weights.cols);
    }
};
...
public:
    ...
    Network(int num_mods, const Hyperparameters& hyperparameters)
        : hp(hyperparameters),
          modules(),
          attention(num_mods, true),
          module_outputs(),
          predictions(hp.batch_size, hp.output_dim, true),
          frozen_modules(num_mods, false)
    {
        std::cout << "Network::Network constructor - start" << std::endl;
        std::cout << "  hp.batch_size in constructor: " << hp.batch_size << std::endl;
        std::cout << "  hp.output_dim in constructor: " << hp.output_dim << std::endl;
        std::cout << "  predictions.size after Tensor construction: " << predictions.size << std::endl;
        modules.reserve(num_mods);
        module_outputs.reserve(num_mods);
        frozen_modules.reserve(num_mods);
        for (int i = 0; i < num_mods; ++i) {
            ...
            // frozen_modules is already initialized with 'false' for each module.
        }
        std::cout << "Network::Network constructor - end" << std::endl;
    }

void forward(const Tensor& input, cudaStream_t stream) {
    std::cout << "Network::forward - start" << std::endl;
    for (int m = 0; m < modules.size(); ++m) {
        std::cout << "Module " << m << " forward start" << std::endl;
        modules[m].forward(input, module_outputs[m], stream);
        std::cout << "Module " << m << " forward end" << std::endl;
    }
    std::cout << "Attention forward start" << std::endl;
    attention.forward(stream);
    std::cout << "Attention forward end" << std::endl;

    // Clear predictions tensor
    checkCudaErrors(cudaMemsetAsync(predictions.data, 0, predictions.size * sizeof(__half), stream));

    std::cout << "Network::forward - scale_and_add loop start" << std::endl;
    dim3 blockDim(256);
    dim3 gridDim((hp.batch_size * hp.output_dim + blockDim.x - 1) / blockDim.x);
    for (int m = 0; m < modules.size(); ++m) {
        std::cout << "scale_and_add_kernel_half launch start - Module " << m << std::endl;
        std::cout << "  module_outputs[m].data: " << module_outputs[m].data << std::endl;
        std::cout << "  attention_weights.data + m: " << (void*)((const __half*)attention.attention_weights.data + m) << std::endl;
        std::cout << "  predictions.data: " << predictions.data << std::endl;
        ...
        scale_and_add_kernel_half<<<gridDim, blockDim, 0, stream>>>(
            ...
            *((const __half*)attention.attention_weights.data + m),
            static_cast<__half*>(predictions.data),
            hp.batch_size * hp.output_dim
        );
        checkCudaErrors(cudaGetLastError());
        std::cout << "scale_and_add_kernel_half launch end - Module " << m << std::endl;
    }
    // Synchronize once after launching all kernels
    checkCudaErrors(cudaStreamSynchronize(stream));
    std::cout << "Network::forward - scale_and_add loop end" << std::endl;
    std::cout << "Network::forward - end" << std::endl;
}
    ...
    void backward(const Tensor& input, const Tensor& grad_predictions, cudaStream_t stream, const Hyperparameters& hp) {
        ...
        for (int m = 0; m < modules.size(); ++m) {
            ...
            compute_dot_product_kernel_half<<<1, 256, 0, stream>>>(
                static_cast<const __half*>(grad_predictions.data),
                static_cast<const __half*>(module_outputs[m].data),
                ((__half*)grad_attention_weights_from_main.data) + m,
    ...
    void update_parameters(float lr, float beta1, float beta2, float epsilon, int step, cudaStream_t stream) {
        for (int m = 0; m < modules.size(); ++m) {
            if (!frozen_modules[m]) {
                update_parameter(modules[m].layer1.linear.weights, lr, beta1, beta2, epsilon, step, stream);
                update_parameter(modules[m].layer1.linear.biases, lr, beta1, beta2, epsilon, step, stream);
                update_parameter(modules[m].layer2.linear.weights, lr, beta1, beta2, epsilon, step, stream);
                update_parameter(modules[m].layer2.linear.biases, lr, beta1, beta2, epsilon, step, stream);
            }
        }
        update_parameter(attention.attention_scores, lr, beta1, beta2, epsilon, step, stream);
    }

    void set_attention_score(int module_index, float score, cudaStream_t stream) {
...
void read_binary(const std::string& path, void* data, size_t size) {
    std::cout << "read_binary: starting for path=" << path << std::endl;
    std::ifstream file(path, std::ios::binary);
    if (!file.is_open()) {
        std::cerr << "read_binary: Failed to open file: " << path << std::endl;
        throw std::runtime_error("Failed to open file: " + path);
    }
    file.read(static_cast<char*>(data), size);
    if (file.gcount() != size) {
        std::cerr << "read_binary: Error reading file: " << path << ". Expected to read " << size << " bytes, but read " << file.gcount() << std::endl;
    ...
    file.close();
    std::cout << "read_binary: finished successfully for path=" << path << std::endl;
}

int main() {
    std::cout << "Main function: program starting..." << std::endl;
    Hyperparameters hp;
    float* h_train_images;
    int* h_train_labels;
    std::cout << "Main function: Allocating host memory for training data..." << std::endl;
    checkCudaErrors(cudaMallocHost(&h_train_images, 50000 * hp.input_size * sizeof(float)));
    checkCudaErrors(cudaMallocHost(&h_train_labels, 50000 * sizeof(int)));
    std::cout << "Main function: Loading training images..." << std::endl;
    read_binary(hp.train_images_path, h_train_images, 50000 * hp.input_size * sizeof(float));
    std::cout << "Main function: Loading training labels..." << std::endl;
    read_binary(hp.train_labels_path, h_train_labels, 50000 * sizeof(int));
    float* h_test_images;
    int* h_test_labels;
    std::cout << "Main function: Allocating host memory for test data..." << std::endl;
    checkCudaErrors(cudaMallocHost(&h_test_images, 10000 * hp.input_size * sizeof(float)));
    checkCudaErrors(cudaMallocHost(&h_test_labels, 10000 * hp.input_size * sizeof(float)));
    std::cout << "Main function: Loading test images..." << std::endl;
    read_binary(hp.test_images_path, h_test_images, 10000 * hp.input_size * sizeof(float));
    std::cout << "Main function: Loading test labels..." << std::endl;
    read_binary(hp.test_labels_path, h_test_labels, 10000 * sizeof(int));

    std::cout << "Main function: Hyperparameter check before Network creation:" << std::endl;
    std::cout << "  hp.batch_size: " << hp.batch_size << std::endl;
    std::cout << "  hp.output_dim: " << hp.output_dim << std::endl;

    if (hp.batch_size <= 0 || hp.output_dim <= 0) {
        std::cerr << "Error: batch_size or output_dim is not positive!" << std::endl;
        return 1;
    }


    std::cout << "Main function: Creating Network..." << std::endl;
    Network network(hp.num_modules, hp);
    std::cout << "Main function: Network created." << std::endl;
    std::cout << "  hp.batch_size: " << hp.batch_size << std::endl;
    std::cout << "  hp.output_dim: " << hp.output_dim << std::endl;
    std::cout << "  network.predictions.data: " << network.predictions.data << std::endl;
    std::cout << "  network.predictions.size: " << network.predictions.size << std::endl;
    ...
    for (int epoch = 0; epoch < hp.num_epochs; ++epoch) {
        ...
        for (int batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
            ...
            float* h_batch_images = h_train_images + batch_idx * hp.batch_size * hp.input_size;

            std::cout << "Batch " << batch_idx << " to_device start" << std::endl;
            input.to_device(h_batch_images, stream, true);
            checkCudaErrors(cudaStreamSynchronize(stream));
            std::cout << "Batch " << batch_idx << " to_device end" << std::endl;

            std::cout << "Batch " << batch_idx << " network.forward start" << std::endl;
            network.forward(input, stream);
            std::cout << "Batch " << batch_idx << " network.forward end" << std::endl;
            checkCudaErrors(cudaStreamSynchronize(stream));

            Tensor grad_predictions(hp.batch_size, hp.output_dim, true);
            float batch_loss_val;

            std::cout << "Batch " << batch_idx << " cross_entropy_loss_forward_kernel_half start" << std::endl;
            ...
            std::cout << "Batch " << batch_idx << " cross_entropy_loss_forward_kernel_half launched" << std::endl;
            checkCudaErrors(cudaGetLastError());
            checkCudaErrors(cudaStreamSynchronize(stream));
            std::cout << "Batch " << batch_idx << " cross_entropy_loss_forward_kernel_half end" << std::endl;


            std::cout << "Batch " << batch_idx << " cross_entropy_loss_backward_kernel_half start" << std::endl;
            ...
            std::cout << "Batch " << batch_idx << " cross_entropy_loss_backward_kernel_half launched" << std::endl;
            checkCudaErrors(cudaGetLastError());
            checkCudaErrors(cudaStreamSynchronize(stream));
            std::cout << "Batch " << batch_idx << " cross_entropy_loss_backward_kernel_half end" << std::endl;


            hp.lambda1 = current_lambda1;
            std::cout << "Batch " << batch_idx << " align_loss.compute start" << std::endl;
            float align_loss_val = align_loss.compute(network.attention.attention_weights, stream, hp);
            std::cout << "Batch " << batch_idx << " align_loss.compute end" << std::endl;
            batch_loss_val += align_loss_val;
            epoch_loss += batch_loss_val;

            std::cout << "Batch " << batch_idx << " network.backward start" << std::endl;
            network.backward(input, grad_predictions, stream, hp);
            std::cout << "Batch " << batch_idx << " network.backward end" << std::endl;
            checkCudaErrors(cudaStreamSynchronize(stream));

            std::cout << "Batch " << batch_idx << " network.update_parameters start" << std::endl;
            adam_step++;
            network.update_parameters(hp.lr, hp.beta1, hp.beta2, hp.epsilon, adam_step, stream);
            std::cout << "Batch " << batch_idx << " network.update_parameters end" << std::endl;
            checkCudaErrors(cudaStreamSynchronize(stream));
        ...
        if (epoch == 5) {
            network.set_attention_score(0, 5.0f, stream);
            network.set_attention_score(1, -2.0f, stream);
        }
        if (epoch == 20) {
        ...
        for (int test_batch_idx = 0; test_batch_idx < num_test_batches; ++test_batch_idx) {
            ...
            float* h_test_batch = h_test_images + test_batch_idx * hp.batch_size * hp.input_size;
            ...
            network.predictions.to_host(static_cast<float*>(h_predictions.data), stream);
            checkCudaErrors(cudaStreamSynchronize(stream));
            int* h_test_batch_labels = h_test_labels + test_batch_idx * hp.batch_size;
            float* h_predictions_host_data = static_cast<float*>(h_predictions.data);
            for (int i = 0; i < hp.batch_size; ++i) {
                int predicted_label = 0;
                float max_val = -std::numeric_limits<float>::infinity();
                for (int j = 0; j < hp.output_dim; ++j) {
                    if (h_predictions_host_data[i * hp.output_dim + j] > max_val) {
                        max_val = h_predictions_host_data[i * hp.output_dim + j];
                        predicted_label = j;
                    }
                }
    ...
    std::cout << "Main function: Training finished." << std::endl;
